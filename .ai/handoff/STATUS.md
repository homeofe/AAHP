> Note (2026-07-14, T-009, claude-opus-4-8): Completed the inline Grounding spec in README section 2.10: added a condensed (<40-line) reference block reproducing the task-type anchor matrix (six task types), the grounded/partially_grounded/ungrounded confidence bands, and the minimum required TRUST.md fields, with a pointer to the full templates/GROUNDING.md. Derived from templates/GROUNDING.md; docs only, no schema or tooling change.

> Note (2026-07-14, T-008, claude-opus-4-8): Added README Section 10 (Multi-Repo and Cross-Repo Handoff): 10.1 propagation model (upstream/consumer/propagation-commit), 10.2 cross-repo handoff pattern introducing the optional additive MANIFEST cross_repo_ref field (repo/commit/handoff_file/relation), 10.3 monorepo per-package handoff, 10.4 version-skew and 12-month deprecation policy. Added cross_repo_ref to schema/aahp-manifest.schema.json (optional, backward compatible; ajv-validated: AAHP's own manifest still valid, a valid ref passes, a bad relation enum fails) and bumped npm version 3.3.0 to 3.4.0 (additive minor) in package.json + package-lock.json.
