...
if [ $# -gt 0 ]; then
    ...
    suite="$1"
    case "$suite" in
        manifest|lint|migrate|migrate-grounding|archive|verify|cli|gates|doctor|anti-entropy|check|init-gates|gates-portability|nonnpm-root)
            echo "Running $suite tests..."
            $BATS "$SCRIPT_DIR/${suite}.bats"
            ;;
        *)
            echo "Unknown suite: $suite" >&2
            echo "Available: manifest, lint, migrate, migrate-grounding, archive, verify, cli, gates, doctor, anti-entropy, check, init-gates, gates-portability, nonnpm-root" >&2
            exit 1
            ;;
    esac
else
